...

{
    ...
    echo "}"
    echo ""
    echo "apply_arp_settings() {"
    echo "    local iface=\"\$1\""
    echo "    sysctl -w \"net.ipv4.conf.\$iface.arp_ignore=1\" >/dev/null"
    echo "    sysctl -w \"net.ipv4.conf.\$iface.arp_announce=2\" >/dev/null"
    echo "}"
    echo ""
    echo "apply_arp_settings all"
